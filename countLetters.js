...
const countLetters = function(stringsToCount) {
  const results = {};
  for (const strings of stringsToCount) {
    if (results[strings]) {
      results[strings] += 1; // if key exists in results object increment value by 1
    } else {
    results[strings] = 1;  // if key does not exist in results object then create the key in the results object with a value of 1
    }
  }
  return results;
}
